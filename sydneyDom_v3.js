...
class creator{
    constructor({tagname,attribute,children,Dom,createState,eventListener , innerHTML} , gen)
    {
        ...
        children.forEach((val , i) =>{
            ...
            if(child) this.element.appendChild(child); //Confirm that child is a valid Node before appending to parent
        })
    }
}

export const render = (vapp , gen = [0]) =>{
    if(vapp.render === undefined || vapp.render === true)
    {
...
class diffAlgo{
    ...
    static startDiffing(params)
    {
        ...
        {
            //Ensure that only rendereable children can be diffed
            const nappChildren = napp.children.filter(child => child.render === undefined || child.render === true);
            const oappChildren = oapp.children.filter(child => child.render === undefined || child.render === true);
            //This will return the extra or overflow children
            let repo = new Array();

            //remove any leftover that might come from oldvapp
            if(oappChildren.length > nappChildren.length)
            {
                const illicit = oappChildren.map((val , id)=> id).filter(id => id >= nappChildren.length).reverse().map(id => {return {name:id , task:"remove"}});
                repo.push(...illicit)
            }
            //remove any leftover that might come from oldvapp

            for(let i = 0;i < nappChildren.length; i++)
            {
                if(oappChildren[i] === undefined)
                {
                    repo.push({name:i , task:"add"});   
                }else
                {
                    //recall main here for child comparison
                    main(oappChildren[i] , nappChildren[i] , element.childNodes[i] , [...gen,i])
                }
            }
            //This will return the extra or overflow children
            ...
            return repo.map(obj => {
                if(obj.task === "add") return [element , (element)=>{
                    if(element.childNodes[obj.name]) element.replaceChild(render(nappChildren[obj.name],gen) , element.childNodes[obj.name])
                    else element.appendChild(render(nappChildren[obj.name],gen))
                }]
...
class createElementClass{
    constructor(tagname,attribute,children,type,createState,mediaQuery,events,genericStyle,innerHTML , legacyName , isRenderable)
    {
        this.mainObj = new Object;
        this.mainObj.tagname = tagname;
        this.mainObj.attribute = attribute;
        this.mainObj.legacy = legacyName;
        this.mainObj.render = isRenderable;
...
export const createElement = (tagname = 'div', attribute = {}, children = [],{type = null,createState = {},mediaQuery = {} , innerHTML = [''],events = {}, genericStyle = [] , legacyName = null , isRenderable = true} = {}) =>{
   return new createElementClass(tagname,attribute,children,type,createState,mediaQuery,events,genericStyle , innerHTML , legacyName , isRenderable).mainObj
}
...
class createState{
    constructor(name,newstate,tenary,tenaryOptions)
    {
        ...
        const commenceDiffAlgorithm = () =>{
            ...
            //Ensure that diffing only happens if newapp is renderable, else , ignore
            if(newapp.render === undefined || newapp.render === true)
            {
                sydDiff(
                    {
                ...
                visualTreeParams['ws'].addEventListener('open', e =>{
                    if(visualTreeParams.mounted && !visualTreeParams.sent && Object.keys(sydDOM).length > 0)
                    {
                        ...
                        const sydvar = Object.keys(SYD_VAR).map(v =>{
                            varNames.push(v);
                            const serialized = JSON.stringify(SYD_VAR[v].get(), (key, value) => {
